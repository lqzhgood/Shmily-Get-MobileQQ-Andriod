...
function fillQQName() {
    const FRIEND_LIST = fs.readJsonSync(path.join(DIST_DIR, './table/friends.json'));
    ...
    if (!config.rightName) {
        config.rightName = FRIEND_LIST.find(v => v.uin === config.rightNum).remark;
    }

    if (!config.leftName) {
        config.leftName = FRIEND_LIST.find(v => v.uin === config.leftNum).remark;
    }
