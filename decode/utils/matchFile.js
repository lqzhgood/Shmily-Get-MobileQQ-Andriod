...
// }
const ASSET_FILE_JSON = getJSON(
    path.join(DIST_DIR_TEMP, "./ASSET_FILE.json"),
    []
);
...
async function matchFile(webDir, fileDir, list, logM = {}, type) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        const matchRes = MATCH_FILE_JSON.find(
            (v) =>
                _.intersectionBy(list, v.aliasList, (s) => s.toLowerCase())
                    .length !== 0
        );
        if (matchRes) {
            ...
            if (!fs.existsSync(filePath))
                Log.errAndThrow("matchFile not exists", filePath, matchRes);
        ...
        if (/^([a-f\d]{32}|[A-F\d]{32})$/.test(l)) {
            ...
            Log.errAndThrow("无法识别的 match 类型", l, logM);
        }
...
async function matchMd5(md5, fileDir, useOriginalName = false) {
    const find = ASSET_FILE_JSON.find(
        (v) => v.md5.toLowerCase() === md5.toLowerCase()
    );
    if (!find) return null;
    ...
    if (tmpExt === ".html") {
        ...
        if (html.includes("长按网址复制")) {
            ...
            if (!realUrl) {
                Log.err(
                    "Down File Error",
                    "realUrl is empty , maybe html template change",
                    url
                );
                throw new Error("");
            }
